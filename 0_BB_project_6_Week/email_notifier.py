    ...
    try:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT id, ticker, signal_type, signal_date FROM signals_log_week
                    WHERE is_active = TRUE
                      AND NOT EXISTS (
                          SELECT 1 FROM signals_sent
                          WHERE signal_id = signals_log_week.id
                      )
...
#==== 25.04.26 / Добавлена отправка сообщения на email при остутствии сигналов ====
    if not signals:
